...
import { LANDING_STEPS, indexLabel, type LandingStep } from "@/data/marketing/landing"
...
function AssembleMock() {
  return (
    <div style={{ display: "flex", gap: 14, height: "100%" }}>
      <Panel style={{ width: 148, padding: 12, display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
        <span className="mk-eyebrow" style={{ fontSize: 10, paddingBottom: 6 }}>Sections</span>
        {["Overview", "Pricing", "Timeline", "Files", "Legal"].map((label, i) => (
          <div
            key={label}
            style={{
              display: "flex",
              ...
              gap: 7,
              fontSize: 12,
              color: i === 1 ? "var(--db-text)" : "var(--db-text-secondary)",
              background: i === 1 ? "var(--db-surface-dim)" : "transparent",
              borderRadius: 6,
              padding: "5px 7px",
            }}
          >
            <div style={{ width: 12, height: 12, borderRadius: 4, background: "var(--db-surface-dim)", border: "1px solid var(--db-border)", flexShrink: 0 }} />
            {label}
          </div>
        ))}
        <span
      ...
      <Panel style={{ flex: 1, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 11 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 4 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: "var(--db-cta-grad)", color: "var(--db-on-accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 600 }}>A</div>
          <span style={{ fontFamily: "var(--font-serif), Georgia, serif", fontSize: 18, letterSpacing: "-0.01em" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, height: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 12, flex: 1, minHeight: 0 }}>
        ...
        <Panel style={{ flex: 2, padding: 14, display: "flex", flexDirection: "column", gap: 4 }}>
          <span className="mk-eyebrow" style={{ fontSize: 10, paddingBottom: 6 }}>Recipients</span>
          {[
            ["Maya Chen", "6:24", true],
            ["Dan Ortiz", "3:12", true],
            ["Priya Nair", "1:48", false],
          ].map(([name, time, active]) => (
            <div key={name as string} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", fontSize: 12.5 }}>
              <div
                style={{
                  width: 20,
                  height: 20,
              ...
              <span style={{ color: "var(--db-text-secondary)" }}>{name}</span>
              <span className="mk-eyebrow" style={{ marginLeft: "auto", fontSize: 10 }}>{time}</span>
              <div
                style={{
                  width: 7,
                  height: 7,
  ...
  return (
    <div style={{ display: "flex", gap: 12, height: "100%" }}>
      <div style={{ flex: 3, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {[
          ["Acme — Q3", "Signed"],
          ["Globex renewal", "In review"],
          ["Initech pilot", "Opened"],
          ["Northwind SOW", "Draft"],
        ].map(([deal, status], i) => (
          <Panel key={status} style={{ padding: 14, display: "flex", flexDirection: "column", gap: 9 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <span style={{ fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{deal}</span>
              <span
                style={{
                  fontSize: 9,
                  fontWeight: 500,
      ...
      <Panel style={{ flex: 2, padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
        <span className="mk-eyebrow" style={{ fontSize: 10 }}>Activity</span>
        {[
          "Maya Chen signed Acme — Q3",
          "Dan Ortiz re-read Pricing",
          "Globex shared your page internally",
          "AI Write drafted Initech pilot",
        ].map((event, i) => (
          <div key={event} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--db-text-secondary)" }}>
            <div
              style={{
                width: 7,
                height: 7,
            ...
            />
            <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{event}</span>
          </div>
        ))}
      </Panel>
    </div>
          ...
          <div role="tablist" aria-label="How it works" style={{ display: "flex", flexWrap: "wrap", gap: 28 }}>
            {LANDING_STEPS.map((s, i) => (
              <button
                ...
                onClick={() => setActive(i)}
              >
                <span className="mk-step-num">{indexLabel(i)}</span>
                <span>{s.title}</span>
              </button>
            ))}
          </div>
