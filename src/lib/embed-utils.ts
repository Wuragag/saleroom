/**
 * Embed provider detection shared by the editor (slash menu, node view), the
 * published renderer (pub-nodes.ts) and the AI sanitizer. Turns the URL a
 * seller pastes into the URL that actually renders inside an iframe — a
 * YouTube watch link, for instance, refuses to be framed; only /embed/ works.
 */
