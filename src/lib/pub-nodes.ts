 ...
 *
 * These are the single source of truth for how custom blocks serialize to
...
// ─────────────────────────────────────────────────────────────────────────────
// Tables — preserve editor-resized column widths and wrap for horizontal
// scrolling on small screens (styles for .pub-table-wrap live in globals.css).
// ─────────────────────────────────────────────────────────────────────────────

const PubTable = Table.extend({
  renderHTML(props) {
    const table = this.parent?.(props) ?? ["table", 0];
    return ["div", { class: "pub-table-wrap" }, table];
  },
});

function cellWidthAttrs(
  colwidth: number[] | null | undefined,
  HTMLAttributes: Record<string, unknown>
) {
  if (!colwidth?.length) return HTMLAttributes;
...
const PubTableCell = TableCell.extend({
  renderHTML({ node, HTMLAttributes }) {
    return ["td", cellWidthAttrs(node.attrs.colwidth, HTMLAttributes), 0];
  },
});

const PubTableHeader = TableHeader.extend({
  renderHTML({ node, HTMLAttributes }) {
    return ["th", cellWidthAttrs(node.attrs.colwidth, HTMLAttributes), 0];
  },
});
...
export const EmbedNodeServer = Node.create({
  name: "embed",
  group: "block",
  atom: true,
  addAttributes() {
...
// ─────────────────────────────────────────────────────────────────────────────
// CTA Button node — left-aligned, accent color
// ─────────────────────────────────────────────────────────────────────────────

export function createCTAButtonNode(accentColor: string) {
  return Node.create({
    name: "ctaButton",
    group: "block",
    atom: true,
...
export function createLogoGridNode(isDark: boolean) {
  return Node.create({
    name: "logoGrid",
    group: "block",
    atom: true,
...
export function createFormBlockNode(isDark: boolean, accentColor: string) {
  return Node.create({
    name: "formBlock",
    group: "block",
    atom: true,
 ...
 * old off-brand rainbow. accentColor may be a raw hex or a CSS var() —
 * color-mix() accepts both, so the editor node views can pass the live
 * --page-accent variable and stay in sync with the published gradients.
 */
...
export function createContactCardNode(isDark: boolean, accentColor: string) {
  return Node.create({
    name: "contactCard",
    group: "block",
    atom: true,
...
export function createBannerNode(accentColor: string) {
  return Node.create({
    name: "banner",
    group: "block",
    atom: true,
...
export function createTestimonialNode(isDark: boolean, accentColor: string) {
  return Node.create({
    name: "testimonial",
    group: "block",
    atom: true,
...
export function createMetricsNode(isDark: boolean, accentColor: string) {
  return Node.create({
    name: "metrics",
    group: "block",
    atom: true,
      ...
      return [
        ...
        mergeAttributes(
          { "data-type": "metrics" },
          {
            ...
            style: `display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;padding:14px;margin:12px 0;border-radius:var(--pub-radius-lg, 16px);background:var(--pub-wash, transparent);`,
          }
        ),
        ...cells,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const SpacerNodeServer = Node.create({
  name: "spacer",
  group: "block",
  atom: true,
  addAttributes() {
...
export const SyncedBlockServerFallback = Node.create({
  name: "syncedBlock",
  group: "block",
  atom: true,
  addAttributes() {
...
export interface PubExtensionOptions {
  isDark?: boolean;
  accentColor?: string;
}

export function buildPubExtensions({
  isDark = false,
  accentColor = "#64748b",
}: PubExtensionOptions = {}) {
  return [
...
/** DOMPurify options for HTML produced by buildPubExtensions(). */
export const PUB_SANITIZE_CONFIG: {
  ADD_TAGS: string[];
  ADD_ATTR: string[];
